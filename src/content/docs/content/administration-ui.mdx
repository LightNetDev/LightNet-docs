...
The Administration UI can also edit LightNet languages through a root-level `languages.json` file.
...
The option name and behavior may still change before the feature is stabilized.
</Aside>
...
If omitted, experimental features stay disabled.
...
Use this when your Git repository should not act as file storage, or when your hosting setup needs larger uploads. For example, Cloudflare Pages and Workers only allow files up to 25 MB.
...
Your Cloudflare account ID. LightNet uses this to construct the R2 API endpoint.
