...
LightNet enables you to manage your content using JSON files. This is great for developers, but not so much for non-technical 
...
We use a tool called [Decap CMS](https://decapcms.org/) to provide the Administration UI. It is a headless CMS that we 
configure to work with LightNet. {/*what is a headless cms? what is a cms at all? is it important to know that?*/}

All changes made are written to your file system or to your git repository if the user interface is used online.
...
supported by the Administration UI). Implementing a better Administration UI is already on our roadmap.
</Aside>

## Add the Administration UI to your project

This is how you add the Administration UI to your LightNet project: {/*isn`t it installed with the sk8-ministries example automatically?*/}
...
1. **Adjust Configuration**: Add your Administration UI configuration to Astro's `astro.config.mjs` file. This is a minimal 
    ...
    export default defineConfig({ 
      integrations: [
        lightnet({
            languages: languages,
            //... {/*shouldn`t here be an example of one integrated language to show the syntax?*/}
        }),
        lightnetDecapAdmin({
            languages: languages,
        })
...
{/*isn't GitLab the only supported platform?*/}
...
A Administrator can access the UI on your website, all requests are send to the Git Host's API.
Every change done on the Administration UI changes files inside your project using Git commits.
Authentication also are handled by the Git Host, so only Persons you give access are able to 
...
5. Save the new Application. GitLab presents you with a Application ID. Copy and store this ID to use it later.
...
<Aside>
Even when you have configured a GitLab backend, your local development server always tries to connect to a local `decap-server`.
</Aside>
