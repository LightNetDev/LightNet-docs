...
The component receives a `slug` property that is the media item's identifier. 
LightNet wraps your component inside its `<Page>` layout.
This example fetches the media item with the given slug and renders the title of the media item: 
...
if (!mediaItem) {
  throw new Error(`Cannot find media item for slug: ${slug}`);
}
---
