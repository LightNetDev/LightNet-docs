...
## Configure the `lightnetLibrary` integration

LightNet Library is an integration built on top of the [Astro](https://astro.build/) web framework.
...
You can pass the following options to the `lightnetLibrary` integration:
...
The logo of your site. This will be used as the logo of the site and the header bar.
...
The path to the logo image. Provide a path to an image inside `src/assets`. Supported formats include `svg`, `jpg`, `png`, and `webp`.  
...
The languages of your site. This will be used to support multiple locales and content languages. 
You should use the same languages as in the `lightnetLibrary` integration. We recommend creating a shared variable that is used in both integrations.
See the [lightnetLibrary configuration](#languages) for more information on the languages structure.
