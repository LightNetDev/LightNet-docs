...
This chapter provides a high-level overview of the approach LightNet uses to build media sites.
...
SSG is a method of pre-building a website by converting raw content (for example, [Markdown, JSON, or YAML](/start-here/recommended-skills) files)
into static [HTML, CSS, and JavaScript](/start-here/recommended-skills) files. 
...
- **Simplified Deployment** - Traditional web applications run on backend infrastructure that includes web servers and database servers.
...
- **Faster Performance** - Pages are pre-rendered during the build process, eliminating unnecessary re-rendering and reducing
    data transmission.
...
The following chapters go into more detail on each of these aspects.
