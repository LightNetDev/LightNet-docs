...
- **Missing referenced content files**: a file reference in a Media Item's `content` array points to a file that is not available. Fix it by uploading or restoring the missing file, or by updating the file reference.
- **Orphaned content files or images**: a file exists, but no Media Item or Category uses it. Review these before deleting them, especially in shared buckets.
...
### Removing unused files with `--fix`
...
With `--r2 --scope content-files`, `--fix` removes unused R2 objects after confirmation. Without `--scope`, it also removes unused local Media Item and Category images. If a saved R2 connection stops working, rerun the command and follow the prompt to update the R2 connection configuration in `lightnet-cli.json`.
