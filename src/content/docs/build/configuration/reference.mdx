...
Provide a locale map keyed by your configured site languages. The default site language is required.
...
An array of fallback language codes used when translations are not available for the current locale.
LightNet iterates over this array in order and uses the first language that provides a matching translation key.
If no match is found, LightNet falls back to the default site language and finally to English.
...
type: `array` \
example: `[ "files.your-site.example" ]` \
required: `false`

The internal domains of the site. This is used to determine whether a URL is internal or external.

For example if you want to link to your files on a different domain e.g. on a subdomain `files.your-site.example`, you can add it to the `internalDomains` array.
A link to `https://files.your-site.example/file.pdf` won't be treated as an external link.
...
Set this to `true` to add a `noindex` robots directive to every page.
This asks search engines not to include your site in search results.

Do not combine this with a `robots.txt` file that disallows all pages.
Search engines must be allowed to crawl a page before they can see its `noindex` directive.

Search engines and other bots are not forced to respect this directive.
...
type: `object` \
example: `{ en: "© {{year}} Your Ministry" }` \
required: `false`

Localized text shown in the built-in LightNet footer, before any configured footer links.
Use this for short site information such as a copyright notice, ministry name, or one-line description.
Provide a locale map keyed by your configured site languages.

Use the `{{year}}` placeholder to insert the current year at build time.
...
Optional links displayed in the built-in LightNet footer.
Here is a [guide on how to use the footer](/build/pages/footer).
...
Defines the link destination. It can be an internal path such as `/about` or an external URL such as `https://lightnet.community`.
...
The text shown for the link. Provide a locale map keyed by your configured site languages.

#### footerLinks: `requiresLocale`
...
If set to `true`, LightNet automatically adds the current locale to internal footer links.
This setting is ignored for external URLs.
