...
It is a large image that takes up the full width of the screen. 
...
### Add a image highlight section

If you have some content to highlight you can use the `ImageSection` component. This component will display a large image with a title, 
a description and a optional link to another page. Here is an example on how to use it:

```astro title="src/pages/[locale]/index.astro"
---
import { Page, ImageSection } from "@lightnet/library/components"
...
For multilingual sites make sure to pass a translated string for the `title`, `text` and `label` props.
