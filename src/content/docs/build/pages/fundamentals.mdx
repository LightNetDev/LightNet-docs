...
This chapter introduces how pages are created with LightNet. Most of the concepts presented here come from Astro,
so use the [Astro documentation](https://docs.astro.build/en/basics/astro-pages/) if you want to dig deeper.

## Defining routes with the pages folder

Routes on your site are determined by the files in the `src/pages` folder.
The file path maps directly to the page route.
For instance, a file at `src/pages/en/about.astro` corresponds to `https://your-site.example/en/about`.

In addition to your own pages, LightNet automatically adds:

- **Homepage Redirect**: Visiting the root path (`https://your-site.example`) redirects to the homepage in the default site language.
...
There are two different types of locale folders: **dynamic** and **static**. You can use both inside the same project.

<Aside type="note">
Dynamic and static routes are based on [Astro's routing concept](https://docs.astro.build/en/guides/routing/).
</Aside>
...
<Aside type="tip">
To access the current locale within a dynamic folder, use `Astro.locals.i18n.currentLocale`.
</Aside>
...
Create separate folders for each locale, such as `src/pages/en/` and `src/pages/de/`.
This approach allows for completely unique page structures per language.
For example, you could build an English homepage that starts with a gallery and a German homepage that starts with text.
...
- The frontmatter (inside `---`) runs during the build and is used for imports and variable definitions.
...
LightNet's `Page` component ensures a consistent layout by adding the header, language selection menu, and navigation menus.
To ask search engines not to index a page, see [Configure search engine indexing](/deploy/search-engines/).
...
This simplifies URLs (e.g., `https://your-site.example/en/about` instead of `https://your-site.example/en/about.html`).
</Aside>
