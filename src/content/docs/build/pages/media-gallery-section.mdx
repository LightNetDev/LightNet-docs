...
description: >
  ...
  We provide a `MediaGallerySection` component that you can use to display your media items. Also we offer a 
...
LightNet provides a `MediaGallerySection` component that displays a grid of media items. Also it offers a 
`getMediaItems` function that you can use to query your media items. 
...
The `MediaGallerySection` component displays your media items in a grid. It shows the title and image of each media item. 
...
<Aside>
Up to now, the `MediaGallerySection` is only available in `maxwidth: "wide"`.
</Aside>