...
The hero image is the eye-catching element users see when they first visit your page.
...
We suggest using a landscape-oriented image that is at least 3000 pixels wide so it looks good on all devices.
The image is automatically optimized for performance. LightNet shows the full width of the image but centers it vertically behind the
...
If your site is multilingual, make sure you pass translated strings for the `title` and `subtitle` properties.
...
By default, the hero section applies a semi-transparent 
dark overlay over the background image to improve text readability. 
Set this option to true to disable the overlay.
...
Additional CSS classes, separated by spaces, to further customize the title's appearance.
...
Additional CSS classes, separated by spaces, to further customize the subtitle's appearance.
...
Additional CSS classes, separated by spaces, to style the hero image. They are applied to an element that overlays the image.
