...
1. **Create a translations file**: Create a empty file in the `src/translations` folder. The file must be named with the language code ending with `.yml` (e.g. `en.yml` for English).
